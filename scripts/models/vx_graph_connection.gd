...
func save():
	...
	if result.size() == 0:
		...
		var insert_query = "INSERT INTO vx_graph_connections (graph_id, connection_id) VALUES (?, ?);"
		execute_query(insert_query, [graph_id, connection_id])

func get_connections_by_graph(graph_id: int) -> Array:
	var query = "SELECT connection_id FROM vx_graph_connections WHERE graph_id = ?;"
